...
#         form = MyInvoiceItemForm(data, instance=item)
#         if form.is_valid():
#             form.save()
...
def invoice_home(request, pk):
    ...
    all_product = Product.objects.only("pk", "name", "price").all()
...
#
#                 # prix = int()
#                 # print("create_invoice: ",prix, quantity)
#
#                 if quantity > 0:
#                     product = Product.objects.get(pk=product_id)
#                     MyInvoiceItem.objects.create(invoice=last_invoice_id, product=product, quantity=quantity,
#                                                  # price=prix
#                                                  )
#         return redirect('facture:invoice_detail', invoice_id=last_invoice_id.id)
#     return redirect('facture:product_list')
